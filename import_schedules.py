...
class Observation:
    ...
    def _translate_to_calendar(self):
        ...
        if observation["observation_type"] in ["RAPID_RESPONSE", "TIME_CRITICAL"]:
            event["project_priority"] = "time_critical"
...
def get_schedule(wema, telescope_id=None, start=None, end=None, limit=1000, observation_state="PENDING"):
    ...
    if response.status_code == 200:
        sched = [x for x in response.json().get("results") if x["state"] == observation_state]
...
def create_latest_schedule_for_subsite(ptr_site):
    ...
    sched = get_schedule(wema, telescope_id, 'PENDING')
